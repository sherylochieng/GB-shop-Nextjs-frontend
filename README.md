...
Paystack setup
Create a Paystack test account and copy the secret key into server/.env.
In the Paystack dashboard (Settings → API Keys & Webhooks) set the webhook URL to https://<your-public-url>/api/paystack/webhook and enable the charge.success and charge.failed events.
Test payments with the test card 4084 0840 8408 4081 — the order page confirms automatically.
...
npm test
Payment flow
Checkout creates the order and reserves stock inside a database transaction
The customer pays on Paystack's hosted page
The browser returns to /api/paystack/verify (settles the order) and is redirected to the order page
The signed webhook independently confirms the charge (idempotent — safe if both run)
A WhatsApp confirmation is sent to the customer's phone
